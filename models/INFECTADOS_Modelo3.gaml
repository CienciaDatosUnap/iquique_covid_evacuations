...
global {
    ...
    string NombreArchivoResultado <- "Log_Infectados0206_d5050_Modelo3";
    ...
    init {
    	...
    	ask cantInfectados50 among Persona where (each.dcInicial = 1 or each.dcInicial = 2 or each.dcInicial = 7 or each.dcInicial = 8){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectados50 among Persona where (each.dcInicial = 5 or each.dcInicial = 6 or each.dcInicial = 11){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
