...
global {
    ...
    string NombreArchivoResultado <- "Log_Infectados0206_Oficial_Modelo1";
	...
	//CENTRO
	int cantInfectadosDC1 <- 20;
	int cantInfectadosDC2 <- 26;
	int cantInfectadosDC7 <- 51;
	int cantInfectadosDC8 <- 66;
	//
	//SUR
	int cantInfectadosDC5 <- 31;
	int cantInfectadosDC6 <- 36;
	int cantInfectadosDC11 <- 34;
	//
    int cantInfectadosCentro <- cantInfectadosDC1+cantInfectadosDC2+cantInfectadosDC7+cantInfectadosDC8;
    int cantInfectadosSur <- cantInfectadosDC5+cantInfectadosDC6+cantInfectadosDC11;
    int cantInicialInfectados <- cantInfectadosCentro + cantInfectadosSur;  //264
    int cantRecuperados <- 4049;
    ...
    init {
    	...
    	ask cantInfectadosDC1 among Persona where (each.dcInicial = 1){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC2 among Persona where (each.dcInicial = 2){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC7 among Persona where (each.dcInicial = 7){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC8 among Persona where (each.dcInicial = 8){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC5 among Persona where (each.dcInicial = 5){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC6 among Persona where (each.dcInicial = 6){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
    	ask cantInfectadosDC11 among Persona where (each.dcInicial = 11){
        	puedeContagiar <- true;
        	prob <- rnd(0.2,0.6);
    	}
